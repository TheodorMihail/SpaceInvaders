...
```json
{
  ...
  "dependencies": {
    "com.theodormihail.basearchitecture": "https://github.com/TheodorMihail/BaseArchitecture.git?path=Assets/UnityPackages/BaseArchitecture#v1.3.0"
  },
  "testables": [ "com.svermeulen.extenject" ]
}
```

The `#v1.3.0` pins the version, matching this project's own manifest. Bump it to pull a newer [release tag](https://github.com/TheodorMihail/BaseArchitecture/tags). The scoped registry resolves the package's dependencies (Zenject, UniTask) from OpenUPM. `testables` enables Zenject's test fixtures. See the [BaseArchitecture README](https://github.com/TheodorMihail/BaseArchitecture#-installation) for its own requirements.
